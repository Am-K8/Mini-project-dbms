...
if(isset($_POST['delete_brand'])) {
    ...
    $select_query="Select *from `user` where fname='$brand_title'";
    $result_select=mysqli_query($con,$select_query);
    $number=mysqli_num_rows($result_select);
    if($number> 0){
     
    // Delete data from the database
    $delete_query = "DELETE FROM `user` WHERE fname = '$brand_title'";
    $result_delete = mysqli_query($con, $delete_query);

    if ($result_delete) {
        echo "<script>alert('Account Deleted TT!');</script>";
    } else {
        echo "<script>alert('Error deleting Account.');</script>";
    }
   } else {
    echo "<script>alert('No Such account is present.');</script>";
...
<div class="input-group w-99 mb-2">
  ...
  <input type="text" class="form-control" name="brand_title" placeholder="Delete Account (Enter first name)" aria-label="Brands" aria-describedby="basic-addon1">
</div>

<div class="input-group w-8 mb-2 m-auto">
  
<input type="submit" class=" bg-secondary border-2 p-2 my-3" name="delete_brand" value="Delete Account" aria-label="Username" aria-describedby="basic-addon1" class=bg-secondary>
